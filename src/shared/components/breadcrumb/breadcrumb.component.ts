...
import {NavigationItem} from 'src/app/admin/theme/layout/admin/navigation/navigation';
...
export class BreadcrumbComponent implements OnInit {
  @Input() type: string;
  @Output() returnStateBreadcrumb: EventEmitter<boolean> = new EventEmitter();
  ...
  async filterNavigation(activeLink) {
    this.returnStateBreadcrumb.emit(true);
    let result = [];
    let title = 'Welcome';
    ...
    if (this.navigationList && this.navigationList.length > 0) {
      this.titleService.setTitle(title + ' | ' + await this.common.trans('app_name'));
    } else {
      this.returnStateBreadcrumb.emit(false);
    }
  }
