...
export class NavContentComponent implements OnInit, AfterViewInit {
    ...
    if (this.windowWidth < 992) {
      ...
      setTimeout(() => {
        ...
        (document.querySelector(
          '#nav-ps-next'
        ) as HTMLElement).style.maxHeight = '100%';
      }, 500);
    }
  }
  ...
  scrollPlus() {
    ...
    if (this.nextConfig.rtlLayout) {
      (document.querySelector(
        '#side-nav-horizontal'
      ) as HTMLElement).style.marginRight = '-' + this.scrollWidth + 'px';
    } else {
      (document.querySelector(
        '#side-nav-horizontal'
      ) as HTMLElement).style.marginLeft = '-' + this.scrollWidth + 'px';
    }
  }
  ...
  fireLeave() {
    ...
    let current_url = this.location.path().split('?')[0];
  ...
  getUserName(): string {
    return this.common.currentUser().name;
  }
